...
- 新增环境变量：
  ...
  - `SATELLITE_REMOTE_SENSING_WORKER_CONCURRENCY=1`
  - `SATELLITE_REMOTE_SENSING_WORKER_QUEUE_SIZE=64`
...
- 说明：
  ...
  - `SATELLITE_REMOTE_SENSING_FUSION_DIRECT_ENABLED` 控制是否启用阶段8直出融合结果并在阶段9跳过 `fusion_stack_envi.py`（默认 `true`）
  - 若现场出现直出路径不稳定，可临时回滚：`SATELLITE_REMOTE_SENSING_FUSION_DIRECT_ENABLED=false`
  ...
  - `SATELLITE_REMOTE_SENSING_WORKER_QUEUE_SIZE` 为内存任务队列长度（推荐 `64`）
...
1. 该参数组是在当前硬件条件下多轮 A/B 后的稳定最优档位。
2. 如需试验新参数，建议一次只改一个变量，并配合 `--clean-scratch` 采样两次以上再下结论。
