...
collect_runtime_config() {
  kubectl -n "${NAMESPACE}" exec "${POD}" -- sh -lc '
    env | grep -E "^SATELLITE_REMOTE_SENSING_(PAN_RPC|PANSHARPEN|FUSION|COREGISTER|DEM_FILE|PERSIST_OUTPUT_DIR|STAGE_TIMEOUT_SECONDS|FUSION_STAGE_TIMEOUT_SECONDS|STAGE_MAX_RETRIES|COMMAND_HEARTBEAT_SECONDS)" | sort
  '
}
